...
rk5_jupyter_cpu = pd.read_csv(script_directory + '/../orbit_csv/y_rk5_tol09_jupyter_cpu.csv', header=None).to_numpy() # (T,N)
...
rk5_cpp_fpga_simulation = pd.read_csv(script_directory + '/../orbit_csv/y_rk45_tol09_fpga_sim.csv', header=None).to_numpy() # (T,N)
...
fpga_jupyter = pd.read_csv(script_directory + '/../orbit_csv/y_fpga_tol09_jupyter.csv', header=None).to_numpy() # (T,N)
...
ax.plot(t_ode5_matlab[::100], euclidean_distance(ode5_matlab[::100, :], utils.kepler_orbit(t_ode5_matlab[::100], r0, v0, mu)[:, :]), label="Distance between exact orbit and rk45 in Matlab with tol=1e-09", color="black")
ax.plot(t_rk5_python[::100], euclidean_distance(rk5_python[::100, :], utils.kepler_orbit(t_rk5_python[::100], r0, v0, mu)[:, :]), label="Distance between exact orbit and rk45 in Python with tol=1e-09", color="red")
...
# ax.plot(t_rk5_cpp[::100], euclidean_distance(rk5_cpp[::100, :], utils.kepler_orbit(t_rk5_cpp[::100], r0, v0, mu)[:, :]), label="Distance between exact orbit and rk45 in C++ with tol=1e-09", color="blue")
# ax.plot(t_rk5_cpp_fpga_simulation[::100], euclidean_distance(rk5_cpp_fpga_simulation[::100, :], utils.kepler_orbit(t_rk5_cpp_fpga_simulation[::100], r0, v0, mu)[:, :]), label="Distance between exact orbit and rk5 in FPGA simulation with tol=1e-09", color="green")
ax.plot(t_fpga_jupyter[::100], euclidean_distance(fpga_jupyter[::100, :], utils.kepler_orbit(t_fpga_jupyter[::100], r0, v0, mu)[:, :]), label="Distance between exact orbit and rk45 in FPGA implementation with tol=1e-09", color="purple")
