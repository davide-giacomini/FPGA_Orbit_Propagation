...
typedef struct {
    ...
    XRunge_kutta_45_uio_map maps[ MAX_UIO_MAPS ];
} XRunge_kutta_45_uio_info;

/***************** Variable Definitions **************************************/
static XRunge_kutta_45_uio_info uio_info;
...
static int uio_info_read_name(XRunge_kutta_45_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRunge_kutta_45_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRunge_kutta_45_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRunge_kutta_45_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRunge_kutta_45_Initialize(XRunge_kutta_45 *InstancePtr, const char* InstanceName) {
	XRunge_kutta_45_uio_info *InfoPtr = &uio_info;
